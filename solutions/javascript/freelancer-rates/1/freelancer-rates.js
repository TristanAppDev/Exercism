...
export function monthRate(ratePerHour, discount) {
  const discRate = applyDiscount(ratePerHour, discount);
  return Math.ceil(discRate * 8 * 22);
}
...
export function daysInBudget(budget, ratePerHour, discount) {
  const discHour = applyDiscount(ratePerHour, discount);
  const dailyRate = discHour * 8;
